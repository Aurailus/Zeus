...
void setupColours(Material material, vec2 texBase, vec2 texCoord, float repeat_scale, float atlas_scale)
{
    if (material.hasTexture == 1)
    {
        vec2 mult = vec2(mod(texCoord.x * repeat_scale - 0.00001, 1), mod(texCoord.y * repeat_scale - 0.00001, 1));

        ambientC = texture(texture_sampler, vec2(texBase.x + (atlas_scale * mult.x), texBase.y + (atlas_scale * mult.y)));
        diffuseC = ambientC;
        speculrC = ambientC;
    }
    else
...
void main()
{
    setupColours(material, texBase, texCoord, repeat_scale, atlas_scale);
